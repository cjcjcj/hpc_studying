...
void BHTree::put_body(const Body* body)
{
    if (node_type == NodeType::EXTERNAL)
    {
        ...
        else
        {
            ...
            {
                if (body->position == old_body->position or node->width < .000001)
                {
                    // if (body->position == old_body->position)
                    //     std::cout << "blat == ";
                    // if (node->width < .000001)
                    //     std::cout << "blat width ";
                    // std::cout << body->position << " " << old_body->position << std::endl;
                }
...
std::ostream& operator<< (std::ostream& os, const BHTree& bht)
{
    os  << "node type: "
        << (bht.node_type == BHTree::NodeType::EXTERNAL ? "EXTERNAL" : "INTERNAL")
        << std::endl
        << "location: " << bht.location
        << std::endl
        << "start_pos: " << bht.node->start_pos << "; width: " << bht.node->width
        << std::endl
        << "mass: " << bht.node->m
        << std::endl;
    if (bht.node_type == BHTree::NodeType::EXTERNAL)
        return os;
    for (int i = 0; i < 8; i++)
        os << *bht.leafs[i];
    return os;
